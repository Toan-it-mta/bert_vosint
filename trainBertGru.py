...
from src.bertGRU import BertClassifier
import json
...
def train_model(opt):
    ...
    # Tạo mô hình BERT
    bert_model = AutoModel.from_pretrained(opt.bert_name)
    tokenizer = AutoTokenizer.from_pretrained(opt.bert_name)

    # Thiết lập tham số
    gru_hiddent_size = opt.gru_hiddent_size
    ...
    num_iter_per_epoch = len(test_dataloader)
    for epoch in range(opt.num_epoches):
        for iter, (document_encode, labels) in enumerate(train_dataloader):
            if torch.cuda.is_available():
                device = torch.device("cuda:0")
                document_encode = document_encode.to(device)
        ...
        if iter % opt.test_interval == 0:
            ...
            te_label_ls = []
            te_pred_ls = []
            for te_feature, te_label in test_dataloader:
