...
def train_model(opt):
    ...
    if torch.cuda.is_available():
        model.cuda()
    model.train()
    train = Dataset(tokenizer, train_dataset[:64], label_mapping, opt.max_token_length, opt.max_sent_length)
    ...
    for epoch in range(opt.num_epoches):
        for iter, (ids, masks, labels) in enumerate(train_dataloader):
            if torch.cuda.is_available():
